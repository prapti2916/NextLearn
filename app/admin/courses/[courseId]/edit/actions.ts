...
export const deleteChapter = async({
  ...
  try {
    ...
    const chapterToDelete = chapters.find((chap: { id: string; }) => chap.id === chapterId);
    ...
    const remainingChapters = chapters.filter((chap: { id: string; }) => chap.id !== chapterId);

    const updates = remainingChapters.map((chap: { id: any; }, index: number) => {
...
export const deleteLesson = async({
  ...
  try {
    ...
    const lessonToDelete = lessons.find((lesson: { id: string; }) => lesson.id === lessonId);
    ...
    const remainingLessons = lessons.filter((lesson: { id: string; }) => lesson.id !== lessonId);

    const updates = remainingLessons.map((lesson: { id: any; }, index: number) => {
