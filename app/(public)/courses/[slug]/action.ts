...
export const enrollInCourseAction = async (courseId: string): Promise<ApiResponse | never> => {
  const user = await requireUser(); // Ensure user session
  ...
  try {
    ...
    if (!course.stripePriceId) {
      return { status: "error", message: "This course is not available for purchase at the moment." };
    }
    ...
    if (error instanceof Stripe.errors.StripeError) {
      return { status: "error", message: "Payment system error. Please try again later" };
    }
